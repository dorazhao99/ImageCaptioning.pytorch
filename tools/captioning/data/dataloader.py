...
class Dataset(data.Dataset):
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            ...
            info_dict = {}
            info_dict['ix'] = ix
            info_dict['id'] = self.info['images'][ix]['cocoid']
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_feat = self.att_loader.get(str(self.info['images'][ix]['cocoid']))
            # Reshape to K x C
            att_feat = att_feat.reshape(-1, att_feat.shape[-1])
            if self.norm_att_feat:
                att_feat = att_feat / np.linalg.norm(att_feat, 2, 1, keepdims=True)
            if self.use_box:
                box_feat = self.box_loader.get(str(self.info['images'][ix]['cocoid']))
        ...
        if self.use_fc:
            try:
                fc_feat = self.fc_loader.get(str(self.info['images'][ix]['cocoid']))
            except:
